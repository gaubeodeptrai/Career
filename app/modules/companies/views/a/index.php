...
$this->title = Yii::t('easyii/companies', 'Companies');
...
<?php if($data->count > 0) : ?>
    ...
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <?php endif; ?>
                <td><a href="<?= Url::to(['/admin/'.$module.'/a/edit/', 'id' => $item->primaryKey]) ?>"><?= $item->company_name ?></a></td>
                <td><?= $item->company_address ?></td>
                <td><?= $item->company_site ?></td>
                <td class="status">
                    <?= Html::checkbox('', $item->status == Companies::STATUS_ON, [
                        'class' => 'switch',
